...
class FileNameLayer():
	...
	#IMPLEMENTS WRITE
	def write(self, path, inode_number_cwd, offset, data, delay):
		'''WRITE YOUR CODE HERE'''
		parent_inode_number = self.LOOKUP(path, inode_number_cwd)
		parent_inode = interface.INODE_NUMBER_TO_INODE(parent_inode_number)
		childname = path.split('/')[-1]
		child_inode_number = self.CHILD_INODE_NUMBER_FROM_PARENT_INODE_NUMBER(childname,parent_inode_number)
		interface.write(child_inode_number, offset, data, parent_inode_number, delay)
		return True
	...
	def link(self, old_path, new_path, inode_number_cwd):
		...
		interface.link(child_inode_number, hardlink_name, hardlink_parent_inode_number)
		return True
	...
	def unlink(self, path, inode_number_cwd):
		...
		childname = path.split('/')[-1]
		child_inode_number = self.CHILD_INODE_NUMBER_FROM_PARENT_INODE_NUMBER(childname,parent_inode_number)
		interface.unlink(child_inode_number,parent_inode_number,childname)
		return True
	#MOVE
